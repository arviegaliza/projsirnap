    ...
    <div className="max-w-5xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
      ...
      <div className="w-full lg:w-96 border rounded p-4 bg-white shadow-md">
        ...
        <form onSubmit={handleBooking} className="flex flex-col gap-2">
          <input type="text" placeholder="Your Name" value={customerName} onChange={e => setCustomerName(e.target.value)} required className="border px-3 py-2 rounded focus:border-emerald-500 focus:ring focus:ring-emerald-200" />
          <input type="tel" placeholder="Phone (optional)" value={phone} onChange={e => setPhone(e.target.value)} className="border px-3 py-2 rounded focus:border-emerald-500 focus:ring focus:ring-emerald-200" />
          <input type="date" value={date} onChange={e => setDate(e.target.value)} required className="border px-3 py-2 rounded focus:border-emerald-500 focus:ring focus:ring-emerald-200" />
          <input type="time" value={time} onChange={e => setTime(e.target.value)} required className="border px-3 py-2 rounded focus:border-emerald-500 focus:ring focus:ring-emerald-200" />
          <input type="number" min="1" placeholder="Guests" value={guests} onChange={e => setGuests(Number(e.target.value))} className="border px-3 py-2 rounded focus:border-emerald-500 focus:ring focus:ring-emerald-200" />
          <button type="submit" className="mt-2 bg-emerald-600 text-white px-3 py-2 rounded hover:bg-emerald-700 transition">Book Now</button>
        </form>
      </div>
    </div>
  );
